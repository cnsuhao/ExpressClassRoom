...
void ConfigFile::save()
{
    ...
    for(std::map<std::string,std::string>::iterator i = m_content.begin();i != m_content.end();i++)
    {
        ...
        {
            file<<"#"<<m_comment[i->first]<<std::endl;
...
void  ConfigFile::addCommont(const std::string &key,const std::string &commont,const std::string& section)
{
    m_comment[makeLower(section+'/'+key)] = commont;
}
